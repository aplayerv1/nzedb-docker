...
RUN cd /tmp && git clone https://github.com/nicolasff/phpredis.git && cd /tmp/phpredis && phpize && ./configure && make && make install && echo "extension=redis.so" > /etc/php/7.2/mods-available/redis.ini
